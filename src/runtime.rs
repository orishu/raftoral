...
pub struct RaftoralGrpcRuntime {
    node_id: u64,
    _transport: Arc<GrpcClusterTransport<Message<WorkflowCommand>, WorkflowCommandExecutor>>,
...
impl RaftoralGrpcRuntime {
    ...
    pub async fn start(config: RaftoralConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let transport = if let Some(channel_builder) = config.channel_builder {
            Arc::new(GrpcClusterTransport::<Message<WorkflowCommand>, WorkflowCommandExecutor>::new_with_channel_builder(
                nodes,
                channel_builder,
            ))
        } else {
            Arc::new(GrpcClusterTransport::<Message<WorkflowCommand>, WorkflowCommandExecutor>::new(nodes))
        };
