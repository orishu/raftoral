...
/// about sub-clusters. When sub-clusters are created via the management runtime,
/// FullNode automatically creates and manages sub-cluster runtime instances.
///
/// The type parameter R specifies the sub-cluster runtime type (e.g., WorkflowRuntime
/// for standalone mode, or WorkflowProxyRuntime for sidecar mode).
pub struct FullNode<R>
where
    R: crate::management::SubClusterRuntime,
{
    /// Management runtime (Layer 7) - manages R sub-clusters
...
// Generic impl block for all FullNode types
impl<R> FullNode<R>
where
    R: crate::management::SubClusterRuntime,
{
    ...
    /// Get a reference to the shared configuration for sub-cluster runtimes
    ///
    /// This allows accessing the shared configuration before execution clusters are created.
    /// All sub-cluster runtime instances share this configuration.
    pub fn shared_config(&self) -> &Arc<Mutex<R::SharedConfig>> {
        &self.shared_config
    }
...
// Workflow-specific impl block for FullNode<WorkflowRuntime>
impl FullNode<crate::workflow::WorkflowRuntime> {
    ...
    pub async fn new(
        ...
        let shared_config = Arc::new(Mutex::new(crate::workflow::WorkflowRegistry::new()));
...
"node_id" => node_id, "leader" => &leader_peer.management_leader_address);
    ...
    /// Get a reference to the shared workflow registry (WorkflowRuntime-specific convenience method)
    ///
...
// Sidecar-specific impl block for FullNode<WorkflowProxyRuntime>
impl FullNode<crate::workflow_proxy_runtime::WorkflowProxyRuntime> {
    /// Create and start a new full node in sidecar mode (bootstrap mode)
    ///
    /// In sidecar mode, workflows are executed by external applications that connect
    /// to the sidecar server via gRPC. The WorkflowProxyRuntime forwards execution
    /// requests to connected apps and receives results back.
    ///
    /// # Arguments
    /// * `node_id` - Unique identifier for this node
    /// * `raft_address` - Network address for Raft communication (e.g., "127.0.0.1:50051")
    /// * `sidecar_address` - Network address for sidecar server (e.g., "127.0.0.1:9001")
    /// * `storage_path` - Optional path for persistent storage (None = in-memory)
    /// * `logger` - Logger instance
    ///
    /// # Returns
    /// A running FullNode instance with sidecar server
    pub async fn new_sidecar(
        node_id: u64,
        raft_address: String,
        ...
        info!(logger, "Creating FullNode in sidecar mode";
            "node_id" => node_id,
        ...
        // Create ProxyConfig with communication channels
        let (proxy_config, exec_req_rx, _exec_res_rx) =
            crate::workflow_proxy_runtime::ProxyConfig::new();
        let shared_config = Arc::new(Mutex::new(proxy_config));

        // Layers 3-7: Create management runtime with WorkflowProxyRuntime
        let config = RaftNodeConfig {
            node_id,
            cluster_id: 0, // Management cluster ID
            snapshot_interval: 100,
        ...
        let (runtime, node) = crate::management::ManagementRuntime::<crate::workflow_proxy_runtime::WorkflowProxyRuntime>::new(
            config,
            transport.clone(),
            mailbox_rx,
            cluster_router.clone(),
        ...
        let add_rx = runtime.add_node(node_id, raft_address.clone()).await?;
        ...
        // Start SidecarServer with channels from ProxyConfig
        let mut sidecar_server = crate::sidecar::SidecarServer::new(
            sidecar_address.clone(),
            logger.clone(),
        );

        // Get execution_result_tx from shared_config to pass to SidecarServer
        let exec_res_tx = {
            let config_guard = shared_config.lock().await;
            config_guard.execution_result_tx.clone()
        };

        sidecar_server.set_proxy_channels(exec_req_rx, exec_res_tx);

        // Start sidecar server in background
        info!(logger, "Starting sidecar server"; "address" => &sidecar_address);
        tokio::spawn(async move {
            if let Err(e) = sidecar_server.start().await {
                eprintln!("Sidecar server error: {}", e);
            }
        });

        // Layer 0: Start gRPC server (for Raft communication only - no workflow management service in sidecar mode)
        let grpc_server = Arc::new(GrpcServer::new(
            cluster_router,
            node_id,
            raft_address.clone(),
        ...
        let addr = raft_address.parse()?;

        info!(logger, "Starting gRPC server (Raft only)"; "address" => &raft_address);
        ...
        let server_handle = tokio::spawn(async move {
            ...
            let _ = Server::builder()
                ...
                // Note: No WorkflowManagementServer in sidecar mode - workflows are submitted via raftoral-client
                .serve(addr)
                .await;
        });

        info!(logger, "FullNode started in sidecar mode";
            "node_id" => node_id,
            "raft_address" => &raft_address,
            "sidecar_address" => &sidecar_address
        );
    ...
    /// Create and start a new full node in sidecar mode (join mode)
    ///
    /// In sidecar mode, workflows are executed by external applications that connect
    /// to the sidecar server via gRPC. This variant joins an existing cluster.
    ///
    /// # Arguments
    /// * `raft_address` - Network address for Raft communication (e.g., "127.0.0.1:50051")
    /// * `sidecar_address` - Network address for sidecar server (e.g., "127.0.0.1:9001")
    ...
    ///
    /// # Returns
    /// A running FullNode instance with sidecar server that has joined the cluster
    pub async fn new_joining_sidecar(
        raft_address: String,
        sidecar_address: String,
        seed_addresses: Vec<String>,
        storage_path: Option<std::path::PathBuf>,
        ...
        info!(logger, "Creating FullNode in sidecar mode (join mode)";
            "raft_address" => &raft_address,
            "sidecar_address" => &sidecar_address,
            "seeds" => ?seed_addresses
        );
        ...
        let (runtime, node) = if should_join_as_voter {
            // Collect existing voter node IDs
            let existing_voters: Vec<u64> = discovered_peers
                .iter()
                .map(|p| p.node_id)
                .collect();
            ...
            crate::management::ManagementRuntime::<crate::workflow_proxy_runtime::WorkflowProxyRuntime>::new_joining_node(
                config,
                transport.clone(),
                mailbox_rx,
                existing_voters,
            ...
            crate::management::ManagementRuntime::<crate::workflow_proxy_runtime::WorkflowProxyRuntime>::new_joining_learner(
                config,
                transport.clone(),
                mailbox_rx,
                existing_voters,
        ...
        // Layer 0: Start gRPC server (for Raft communication only)
        let grpc_server = Arc::new(GrpcServer::new(
            cluster_router,
            node_id,
            raft_address.clone(),
        ...
        let server_handle = tokio::spawn(async move {
            ...
            if let Err(e) = Server::builder()
                ...
                // Note: No WorkflowManagementServer in sidecar mode
                .serve(addr)
                .await
            {
                eprintln!("gRPC server failed to start on {:?}: {:?}", addr, e);
        ...
        info!(logger, "FullNode started in sidecar mode (join mode)";
            "node_id" => node_id,
            "raft_address" => &raft_address,
            "sidecar_address" => &sidecar_address
        );
    ...
    /// Get a reference to the proxy configuration (sidecar-specific convenience method)
    ///
    /// This allows accessing the proxy configuration for the sidecar mode.
    pub fn proxy_config(&self) -> &Arc<Mutex<crate::workflow_proxy_runtime::ProxyConfig>> {
        &self.shared_config
    }
}
